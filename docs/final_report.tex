...
\paragraph{Appendix figure analysis.} In NYU, RGBD predictions closely track label gradients: after thresholding the free-space boundary is smooth and corridors remain connected, consistent with planner success $\geq$0.969. The RGB-only model exhibits mid-cost flattening, which erodes corridor width and occasionally disconnects free-space, matching the observed 0.79--0.89 success. In KITTI, the post-fix predictions regain dynamic range; binarized maps reveal connected traversable regions at $\tau\in[0.5,0.6]$. Residual high-cost islands near the boundary (visible in the difference map) explain the remaining failures ($\approx$16--27\%). These are most impactful when start/goal land in or adjacent to such pockets.
